...
//     db.connect('mongodb://localhost/portal');
//
// record.js will emit three event with respect to connection
//    
...
//     });
//
// record module will keep track of its connection so you cannot connect 
// multiple times per application.  It is expected that the application
// connects once and then disconnects at termination
//
//     db.disconnect(function(err) {
//        ...
//     });
//
// Disconnecting before connecting raises an error.  Connecting twice raises an error
//
// Once you are connected you can get the whole master record
//    
//     db.getMaster(owner, options, function(err, master) {
//        ...
//     );
//    
// master contains the individual sections.  owner is a unique patient specific key
// 
//     var allergies = master.allergies;
//     var demographics = master.demographics;
//
// you can use section specific helper methods to load individual sections
//
//     db.getAllergies(owner, function(err, allergies) {
//        ...
//     );
//
// you can load a subset of sections
//   
//     var options = {section: ['demographics', 'allergies']}
//     db.getMaster(owner, options, function(err, master) {
//        ...
//     );
//
...
//
//     var numAllergies = allergies.length;
//     allergies.forEach(function(a) {
//         console.log(a.severity);
//     });
//     allergy = allergies[0]
//
// each allergy object is described by the schema in respective section modules
//    
//     var allmodule = require('allergies');
//     var allschema = allmodule.schema
//     console.log(allschema.severity);         // prints [Function:String]
//     console.log(typeof allergy.severity)     // prints string
//
// allmodule.schema is not mongoose Schema object but it is used by record module
// to create one.  There is a special "record_id" field in allschema.  This field
// is important for saving.
// 
// After you manipulate master record you can save it back
//    
//     db.putMaster(owner, master, options, function(err) {
//         ...
//     });
//
// If a section is missing from master no update is done for that section.  You can remove
// the whole section by setting the master property to null
//
//     master.allergies = null;
//     db.putMaster(owner, master, {delstrategy: 'whennull'}, function(err) {
//        ...
//     });
// 
// default for options.delstrategy is 'never'.  'whenunspecified' is also available.
//
// You can save individual sections by passing an array of specific objects
//
//     var allergies = [allergy, allergy_other];
//     db.putAllergies(owner, allergies, options, function(err) {
//       ...
//     });
//
// If allergy has an "record_id" field it replaces the original record otherwise a new 
// allergy is added.  Only those in the allergies are added or replaced.  You can also 
// choose to replace all the allergies
//
//     db.putAllergies(owner, allergies, {replace: true}, function(err) {
//        ...
//     });
//    
// putMaster also respects options.replace.
//
// put methods returns an error if the records do not conform to the schema specified in 
// model method.
//
//     allergy.nonsense = 'value';
//     var allergies = [allergy];
//     db.putAllergies(owner, allergies, options, function(err) {
//       ...
//     });
//
// or if the field is expected to exist but does not
//
//     allergy.name = null;
//     var allergies = [allergy];
//     db.putAllergies(owner, allergies, options, function(err) {
...
//     });
//
// model modules also provide a validate method which can be called 
// independent of the put methods
//
//     allergy.delete('name');
//     var err = allmodule.validate(allergy)
//     console.log(err && err.message) ...  // Name field is missing.
//
//  model module provides a allergy object whcih does the validation on the fly
//      
//      var allergy = allmodule.allergy();
//
// This object validates integrity on the fly      
//      
//      allergy.nonsense = 'value';
//
// raises an exception.  It also provides a validate method that can be 
// used instead of module validate.  You can pass a spec object which initializes model 
// objects
//
//      var allergy = allmodule.allergy(spec);
//
// only fields in the schema is copied.  Extra fields are ignored.
//
// You can specify get methods to return objects of the kind of the model
//
//      db.getAllergies(owner, {modelobj: true}, function(err, master) {
//       ...
//
// These model objects are expected to get additional method such as getCCDA or write(CCDA).
...
var getSection; // forward definition
var putSection; // forward definition
...
DBInfo.prototype.connect = function(uri) {
    ...
    c.on('connected', function() {
        ...
        that.OwnerModel = c.model(that.ownerDbTitle, that.ownerSchema);
...
DBInfo.prototype.dropAll = function(callback) {
    ...
    if (this.connection) {
        ...
        var count = 0;
        var that = this;
        var updateResult = function(sectionTitle, rhs) {
            ++count;
            result[sectionTitle] = rhs;
        ...
        this.dropOwner(function(err) {
            ...
            that.sectionDbTitleList.forEach(function(sectionTitle) {
                ...
                if (r) {
                    ...
                    r.drop(function(err) {
                        updateResult(sectionTitle, err);
                    });
                } else {
                    updateResult(sectionTitle, null);
                }
            });
        });
    } else {
        ...
        ownerRecord.save(function(err) {
            if (! err) {
                Object.keys(existingSectionIds).forEach(function(sectionName) {
                    ...
                    dbInfo.SectionModel[sectionName].findByIdAndRemove(existingId);
                });
            }
            callback(err);
        });
...
var putMaster = exports.putMaster = function(owner, inputRecord, options, callback) {
    ...
    var updateIds = function(sectionName, rhs) {
        ids[sectionName] = rhs;
            ...
            createSection(sectionName, owner, sectionRecord, function(err, id) {
                if (ids) {
                    if (err) {
                        ...
                        updateIds(sectionName, id);
                    }
                }
            });
        } else if (ids) {
            updateIds(sectionName, null);
        }
    });
};
...
putSection = function(sectionName, owner, sectionRecord, callback) {
...
getSection = function(sectionName, owner, callback) {
